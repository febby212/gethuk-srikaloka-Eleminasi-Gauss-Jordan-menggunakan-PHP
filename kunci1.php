...
<body class="relative antialiased bg-gray-100">
    ...
    <section class="mt-12 mb-12 lg:mb-0 md:container md:mx-auto block rounded-lg shadow-lg bg-white px-6 py-12 md:px-12">
        ...
        if (isset($_SESSION['jumlah_persamaan'])) {
            ...
            echo '<h3 class="text-center font-semibold">Tampilan Matrik Pertama</h3>';
        ...
        {
            echo '<div class="flex justify-center mt-0 lg:mb-0 md:container md:mx-auto block rounded-lg shadow-lg bg-white px-6 py-12 md:px-12">';
            echo '<table class="text-center">';
            $rows = count($koefisien);
            for ($i = 0; $i < $rows; $i++) {
                $cols = count($koefisien[$i]);
                echo '<tr class="bg-white border-2 border-grey-300">';
                for ($j = 0; $j < $cols; $j++) {
                    echo '<td class="px-6 py-4 whitespace-nowrap text-sm font-medium border-2 border-grey-300">';
                    echo $koefisien[$i][$j] . '   ';
                    echo '</td>';
                }
                echo '</tr>';
        ...
        function ubah($persamaan)
        {
            ...
            for ($i = 0; $i < $persamaan; $i++) {
                $persamaan_pivot = $i + 1;
                echo '<br>';
                echo '<div class="text-center font-semibold">';
        ...
        function kesimpulan($jumlah_persamaan)
        {
            global $koefisien;
            echo '<br>';
            echo '<div class="text-center font-semibold mt-0 lg:mb-0 md:container md:mx-auto block rounded-lg shadow-lg bg-yellow-200 px-3 py-3 md:px-6"">';
            echo 'Sehingga: ';
            for ($i = 0; $i < $jumlah_persamaan; $i++) {
                echo '<br>X<sub>' . $i . '</sub>: ';
                for ($j = 0; $j < $jumlah_persamaan + 1; $j++) {
        ...
        <div class="mt-12">
            <h2 class="text-base font-bold text-left text-gray-800">
                *)X<sub>0</sub> merupakan varian gethuk coklat
            </h2>
            <h2 class="text-base font-bold text-left text-gray-800">
                *)X<sub>1</sub> merupakan varian gethuk keju
            </h2>
            <h2 class="text-base font-bold text-left text-gray-800">
                *)X<sub>2</sub> merupakan varian gethuk cappucino
            </h2>
        </div>
    </section>
    <br><br><br><br>
